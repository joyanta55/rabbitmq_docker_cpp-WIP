...
// amqp_lib.h includes all the necessary headers to do the operations.
// amqp_lib.cpp contains a wrapper for the AMQP CPP.
...
void messageCallback(const AMQP::Message &message, uint64_t deliveryTag,
  ...
  std::cout << "Received message: " << msg << "Delivery tag " << deliveryTag
            << std::endl;
